...
public class ShopController {
	...
	public String addToCartPost(CartVO vo, HttpSession session, String flag) {
		...
		if(cartVO != null) {	// 현재 구매상품이 기존 구매상품과 같은상품을 구매했을때만 이곳으로 들어온다. 즉, 이때 구매한 상품에 대하여 수량만 증가처리
			...
			String[] newQuantities = vo.getQuantity().split(",");			// 지금(현재) 구매한 장바구니의 수량을 가져와서 ','를 기준으로 분리 
			String[] basicQuantities = cartVO.getQuantity().split(",");	// 기존에 구매했었던 장바구니의 수량을 가져와서 ','를 기준으로 분리
			...
			*/
	        ...
	        for (int i = 0; i < newQuantities.length; i++) {
	            int updatedQuantity = Integer.parseInt(newQuantities[i]) + Integer.parseInt(basicQuantities[i]);
	            updatedQuantities.append(updatedQuantity);
	            if (i < newQuantities.length - 1) {
	                updatedQuantities.append(",");
	            }
	        }
	        
			//vo.setQuantity(strNums);	// 새롭게 정비한 수량을 다시 vo에 set시켜준다.
	        
	        vo.setQuantity(updatedQuantities.toString());
		...
		if(res != 0) {
			if(flag.equals("order")) return "redirect:/msg/cartOrderOk";	// 'flag'에 'order'이 넘어올경우는 '장바구니보기'로 가지않고 바로 '배송지'처리창으로 보낸다.(지금은 처리하지않음-장바구니보기로보냄)
			else return "redirect:/msg/cartInputOk";	// '장바구니담기'버튼클릭시 장바구니에 구매상품의 정보를 담은후, 여기에서는 다시 쇼핑하기로 보냈다.(장바구니 보기로 이동해도 됨)
		}
		else return "redirect:/msg/cartOrderNo";
	}
	
	// 장바구니 보기
	@RequestMapping(value="/productCart", method=RequestMethod.GET)
	public String productCartGet(HttpSession session, CartVO vo, Model model) {
		String userId = (String) session.getAttribute("sUid");
		List<CartVO> productCartVOS = shopService.getProductCart(userId);
		
		if(productCartVOS.size() == 0) return "redirect:/msg/cartEmpty";
	...
	public String productCartDeleteGet(int productIdx) {
		return shopService.setProductCartDelete(productIdx) + "";
	}
}
